...
$(document).ready(function(){
    ...
    % if app.USE_NZBS:
        ...
        $(event.currentTarget).addProvider('${cur_newznab_provider.get_id()}', '${cur_newznab_provider.name}', '${cur_newznab_provider.url}', '${cur_newznab_provider.api_key}', '${",".join(cur_newznab_provider.cat_ids)}', ${int(cur_newznab_provider.default)}, show_nzb_providers);
        % endfor
    % endif
    % if app.USE_TORRENTS:
        % for cur_torrent_rss_provider in app.torrentRssProviderList:
        $(event.currentTarget).addTorrentRssProvider('${cur_torrent_rss_provider.get_id()}', '${cur_torrent_rss_provider.name}', '${cur_torrent_rss_provider.url}', '${cur_torrent_rss_provider.cookies}', '${cur_torrent_rss_provider.title_tag}');
        % endfor
    % endif
});
